...
enum mc_symbol_table_mode {
    MC_SYMBOL_TABLE_MODE_FUNCTION,
    MC_SYMBOL_TABLE_MODE_PROGRAM,
};

enum mc_symbol_table_status {
    MC_SYMBOL_TABLE_ARGSTAT_STDIN,
    MC_SYMBOL_TABLE_ARGSTAT_FILES,
    MC_SYMBOL_TABLE_ARGSTAT_ERROR,
    MC_SYMBOL_TABLE_ARGSTAT_FILE_NOT_FOUND,

};

struct mc_symbol_table_options {
    bool write_to_file;
    char *output_file;
    bool print_help;
    bool limited_scope;
    char *function;
    enum mc_symbol_table_mode mode;
};

struct mc_symbol_table_command_line_parser {
    struct mc_symbol_table_options *options;
    struct mc_symbol_table_program_arguments *arguments;
};

struct mc_symbol_table_program_arguments {
    int size;
    char **args;
};
...
// Print usage of mc_symbol_table
...
// Parse the command line from mc_symbol_table
struct mc_symbol_table_command_line_parser *parse_command_line(int argc, char *argv[]);

// Parse the command line options from mc_symbol_table
struct mc_symbol_table_options *parse_options(int argc, char *argv[]);

// Parse the command line arguments from mc_symbol_table
struct mc_symbol_table_program_arguments *parse_arguments(int argc, char *argv[]);

// Clean up command line parsing results
void mc_symbol_table_delete_command_line_parser(struct mc_symbol_table_command_line_parser *command_line);

// Check if stdin or files was supplied
enum mc_symbol_table_status mc_symbol_table_check_args(struct mc_symbol_table_command_line_parser *command_line);
...
struct mcc_parser_result *mc_symbol_table_merge_results(struct mcc_parser_result* array, int size);
...
int main(int argc, char *argv[])
{
    ...
    struct mc_symbol_table_command_line_parser *command_line = parse_command_line(argc, argv);
    if (command_line == NULL) {
        mc_symbol_table_delete_command_line_parser(command_line);
        return EXIT_FAILURE;
    }
    ...
    enum mc_symbol_table_status argument_status = mc_symbol_table_check_args(command_line);

    // Args were malformed
    if (argument_status == MC_SYMBOL_TABLE_ARGSTAT_ERROR){
        print_usage(argv[0]);
        mc_symbol_table_delete_command_line_parser(command_line);
        return EXIT_FAILURE;
    }

    if (argument_status == MC_SYMBOL_TABLE_ARGSTAT_FILE_NOT_FOUND){
        printf("-------------------------------------------\n");
    ...
    if (argument_status == MC_SYMBOL_TABLE_ARGSTAT_STDIN){
    ...
    if (argument_status == MC_SYMBOL_TABLE_ARGSTAT_FILES){
        ...
        result = *(mc_symbol_table_merge_results(parse_results, command_line->arguments->size));
    }
    ...
    if (command_line->options->mode == MC_SYMBOL_TABLE_MODE_FUNCTION) {
...
void print_usage(const char *prg)
{
    ...
    printf("Utility for displaying the generated symbol tables.\n");
    printf("Errors are reported on invalid inputs.\n\n");
...
struct mc_symbol_table_options *parse_options(int argc, char *argv[])
{
    struct mc_symbol_table_options *options = malloc(sizeof(*options));
    if (options == NULL) {
    ...
    options->mode = MC_SYMBOL_TABLE_MODE_PROGRAM;
    if (argc == 1) {
    ...
    while ((c = getopt_long(argc, argv, "o:hf:t",long_options,NULL)) != -1) {
        switch (c) {
            ...
            case 'f':
                ...
                options->mode = MC_SYMBOL_TABLE_MODE_FUNCTION;
...
struct mc_symbol_table_program_arguments *parse_arguments(int argc, char *argv[])
{
    ...
    struct mc_symbol_table_program_arguments *arguments = malloc(sizeof(*arguments));
    if (arguments == NULL) {
...
struct mc_symbol_table_command_line_parser *parse_command_line(int argc, char *argv[])
{
    ...
    struct mc_symbol_table_options *options = parse_options(argc, argv);
    if (options == NULL) {
        perror("parse_command_line: parse_options");
        return NULL;
    }

    struct mc_symbol_table_command_line_parser *parser = malloc(sizeof(*parser));
    if (parser == NULL) {
        perror("parse_command_line: malloc");
        return NULL;
    }

    struct mc_symbol_table_program_arguments *arguments = parse_arguments(argc, argv);
...
enum mc_symbol_table_status mc_symbol_table_check_args(struct mc_symbol_table_command_line_parser *command_line)
{
    ...
    if (command_line->arguments->size == 0){
        return MC_SYMBOL_TABLE_ARGSTAT_ERROR;
    }

    // 1 argument -> stdin ?
    if (command_line->arguments->size == 1 && strcmp(*(command_line->arguments->args), "-") == 0) {
        return MC_SYMBOL_TABLE_ARGSTAT_STDIN;
        ...
        while (i < command_line->arguments->size) {
            if (strcmp(*(command_line->arguments->args + i), "-") == 0) {
                ...
                if (access(*(command_line->arguments->args + i), F_OK) == -1){
                    return MC_SYMBOL_TABLE_ARGSTAT_FILE_NOT_FOUND;
                }
            }
            i++;
        }
        // 1+ arguments, all of which are files
        return MC_SYMBOL_TABLE_ARGSTAT_FILES;
